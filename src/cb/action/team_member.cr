...
module CB::Action
  ...
  # actions.
  abstract class TeamMemberAction < APIAction
    eid_setter team_id
  ...
  class TeamMemberAdd < TeamMemberAction
    property role : String = CB::TEAM_ROLE_MEMBER
    ...
    def validate
      ...
      raise Error.new("invalid role '#{@role}'") unless CB::VALID_TEAM_ROLES.includes? @role
    ...
    def run
      ...
      team_member = client.create_team_member(
        @team_id,
        CB::Client::TeamMemberCreateParams.new(email.to_s, role),
      )
  ...
  class TeamMemberList < TeamMemberAction
    def validate
      check_required_args do |missing|
        missing << "team" unless team_id
      end
  ...
  class TeamMemberInfo < TeamMemberAction
    def run
  ...
  # however, the account ID will take precedence over the email. Therefore, if
  # supplying both will raise a validation error.
  class TeamMemberUpdate < TeamMemberAction
    setter role : String?
  ...
  # however, the account ID will take precedence over the email. Therefore, if
  # supplying both will raise a validation error.
  class TeamMemberRemove < TeamMemberAction
    def run
      validate_account_email
